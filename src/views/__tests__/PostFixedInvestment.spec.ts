...
describe("PrefixedInvestment", () => {
    ...
    it("updates the inflation rate to percentage properly", () => {
        const computedTrigger = { expectedCdi: 12.60 }

        expect(PostFixedInvestment.computed.percentageCdi.call(computedTrigger)).toBe(0.126)
    })
    ...
    it("updates the equivalent postfixed LCI/LCA when typing values", async () => {
        ...
        expect(wrapper.find("#postfixed-lci-card .card-text").text()).toBe('80.00%')
    })
    ...
    it("updates the equivalent postfixed CDB when typing values", async () => {
        ...
        expect(wrapper.find("#postfixed-cdb-card .card-text").text()).toBe('109.16%')
    })
})
